...
export default function PlayoffResultView({
  data,
  links,
}: {
  data: PlayoffShareData;
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <TeamName name={data.champion.name} teamId={links?.[data.champion.name]} className="block font-display text-3xl font-black text-white" />
        ...
        {data.path.map((p, i) => (
          <div key={i} className="px-4 py-3 flex items-center gap-3">
            ...
            <span className="flex-1 text-sm font-bold text-white truncate">
              def. <TeamName name={p.opp} teamId={links?.[p.opp]} />
            </span>
